...
if __name__ == "__main__":
    inicializar_db()
    enviar_telegram("🚀 *¡BOT AVANZADO EN LA NUBE ACTIVADO!* 🚀\n\nMonitoreando Ligas Europeas de 16:00 a 24:00 sin depender de Termux.\n- Filtro: Cuotas altas (>2.10)\n- Estimador de tarjetas amonestaciones.")
    while True:
        h = datetime.now().hour
        if 16 <= h < 24:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Analizando...")
            procesar()
            time.sleep(INTERVALO_CHECK)
        else:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Standby horario activo.")
            time.sleep(900)
